...
router.post('/', upload.fields([
  ...
  try {
    ...
    try {
      audioUrl = await uploadToS3(files.audioFile[0], `${walletAddress}/songs`);
      coverImageUrl = await uploadToS3(files.coverImage[0], `${walletAddress}/covers`);
    } catch (uploadError) {
...
router.get('/library', async (req, res) => {
  try {
    ...
    if (!profileUser) {
      // User not found - error thrown below
      return res.status(404).json({ error: 'User not found' });
    }
