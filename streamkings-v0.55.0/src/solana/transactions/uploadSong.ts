// uploadSong.ts - Platform action for uploading songs
...
export async function uploadSong({
  ...
  feeOwner,
  configAccount
}: UploadSongParams): Promise<string> {
  ...
  try {
    ...
    const platformActionTx = await program.methods
      .platformAction("upload_song", new anchor.BN(20000000000))
      .accounts({
    ...
    if (err instanceof Error) {
      console.error('[UploadSong] Transaction failed:', err.message);
    }
    throw err;
  }
} 