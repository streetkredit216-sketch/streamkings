...
export function logEnvironmentInfo(config: EnvironmentConfig): void {
  // Only log non-sensitive environment info
  console.log('🌍 Environment Configuration:');
  console.log(`  NODE_ENV: ${config.NODE_ENV}`);
  console.log(`  AWS_REGION: ${config.AWS_REGION}`);
  console.log(`  FRONTEND_URL: ${config.FRONTEND_URL}`);
  console.log(`  LOG_LEVEL: ${config.LOG_LEVEL}`);
  console.log(`  AWS credentials: ${config.AWS_ACCESS_KEY_ID ? 'configured' : 'missing'}`);
  
  if (config.MEMCACHED_URL) {
    console.log(`  MEMCACHED_URL: configured`);
  }
  
  if (config.ELASTICSEARCH_URL) {
    console.log(`  ELASTICSEARCH_URL: configured`);
  }
