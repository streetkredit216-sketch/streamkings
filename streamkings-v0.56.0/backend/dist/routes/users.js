...
router.post('/:walletAddress/profile-pic', upload.single('file'), async (req, res) => {
    try {
        ...
        const imageUrl = await (0, s3_1.uploadToS3)(file, `${walletAddress}/profile-pics`);
...
router.post('/:walletAddress/banner', upload.single('file'), async (req, res) => {
    try {
        ...
        const imageUrl = await (0, s3_1.uploadToS3)(file, `${walletAddress}/banners`);
        const user = await prisma.user.update({
            where: { walletAddress },
...
router.post('/register', async (req, res) => {
    try {
        ...
        if (!username || !role || !walletAddress) {
            console.log('Missing required fields:', { username, role, walletAddress });
        ...
        try {
            await (0, s3_2.createUserFolders)(walletAddress);
        }
        catch (s3Error) {
        ...
        if (profilePic && profilePic.startsWith('data:image')) {
            try {
                ...
                profilePicUrl = await (0, s3_1.uploadToS3)(fileData, `${walletAddress}/profile-pics`);
            }
            catch (uploadError) {
