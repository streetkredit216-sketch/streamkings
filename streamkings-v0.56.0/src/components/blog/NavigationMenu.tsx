...
export default function NavigationMenu({ 
  ...
  // Use IntersectionObserver to only make nav sticky when it reaches its natural position
  // This prevents the nav from covering the radio player on mobile
  useEffect(() => {
    // Use the sentinel from BlogSection (positioned at top of BlogSection)
    const sentinel = document.getElementById('blog-section-sentinel');
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          // When sentinel leaves viewport (scrolled past), make nav sticky
          // When sentinel is visible, nav is not sticky (in its natural position)
  ...
  return (
    <div className="flex flex-col w-full">
      <div 
        ref={navRef}
        className={`${isSticky ? 'sticky top-0' : ''} z-[45] w-full bg-zinc-900 border-b border-white/10`}
      >
