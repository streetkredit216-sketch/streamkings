...
import testTokenInfo from '../programs/testTokenInfo.json';
// Debug imports removed - will add back if needed

// Removed manual discriminator function - now using Anchor's built-in methods
...
export async function createBlog({
  ...
  feeOwner,
  configAccount
}: CreateBlogParams): Promise<string> {
  ...
  try {
    ...
    const platformActionTx = await program.methods
      .platformAction("blog", new anchor.BN(20000000000))
      .accounts({
    ...
    if (err instanceof Error) {
      console.error('[CreateBlog] Transaction failed:', err.message);
    }
    throw err;
  }
}
