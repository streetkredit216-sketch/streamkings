// follow.ts - User action for following
...
export async function follow({
  ...
  try {
    ...
    const userActionTx = await program.methods
      .userAction("follow", new anchor.BN(100000000000)) // 100 tokens as per config
      .accounts({
    ...
    if (err instanceof Error) {
      console.error('[Follow] Transaction failed:', err.message);
    }
    throw err;
  }
} 