// streamSong.ts - User action for streaming songs
...
interface StreamSongParams {
  connection: Connection;
  wallet: WalletContextState;
  fromOwner: PublicKey;        // owner of sender ATA
  receiverOwner: PublicKey;    // owner of receiver ATA (where tokens go)
...
export async function streamSong({
  ...
  feeOwner,
  configAccount
}: StreamSongParams): Promise<string> {
  // Basic runtime guards
  if (!wallet?.publicKey || !wallet?.signTransaction || !wallet?.signAllTransactions) {
    throw new Error('Invalid wallet: missing adapter or not connected');
  }
  [fromOwner, receiverOwner, feeOwner, configAccount, STREET_CREDIT_MINT, PROGRAM_ID].forEach((p, i) => {
  ...
  try {
    ...
    const receiverTokenAccount = await getToken2022Ata(STREET_CREDIT_MINT, receiverOwner);
    ...
    await checkAndAddCreateAta(receiverTokenAccount, receiverOwner);
    ...
    const userActionTx = await program.methods
      .userAction("stream", new anchor.BN(5000000000)) // 5 tokens as per config
      .accounts({
        from: fromTokenAccount,
        receiverWallet: receiverTokenAccount,
    ...
    tx.add(...userActionTx.instructions);
    ...
    if (err instanceof Error) {
      console.error('[StreamSong] Transaction failed:', err.message);
    }
    throw err;
  }
} 