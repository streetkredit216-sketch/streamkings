// comment.ts - User action for commenting
...
export async function comment({
  ...
  try {
    ...
    const userActionTx = await program.methods
      .userAction("comment", new anchor.BN(50000000000)) // 50 tokens as per config
      .accounts({
    ...
    if (err instanceof Error) {
      console.error('[Comment] Transaction failed:', err.message);
    }
    throw err;
  }
} 