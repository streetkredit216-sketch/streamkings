// unfollow.ts - User action for unfollowing
...
export async function unfollow({
  ...
  try {
    ...
    const userActionTx = await program.methods
      .userAction("unfollow", new anchor.BN(20000000000)) // 20 tokens as per config
      .accounts({
    ...
    if (err instanceof Error) {
      console.error('[Unfollow] Transaction failed:', err.message);
    }
    throw err;
  }
} 